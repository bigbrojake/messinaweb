    ...
    useEffect(() => {
        let ctx = gsap.context(() => {
            metrics.forEach((metric, i) => {
                let el = document.getElementById(`metric-val-${i}`);
                if (el) {
    ...
    return (
        <section ref={containerRef} className="py-24 px-6 max-w-7xl mx-auto w-full border-t border-gray-100 mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {metrics.map((m, i) => (
                    <div key={i} className="bg-textDark text-white p-12 rounded-[3rem] text-center flex flex-col items-center justify-center shadow-max">
                        <div className="flex items-baseline mb-4 font-heading font-bold overflow-hidden" style={{ minHeight: '80px' }}>
                            <span className="text-4xl text-accent">{m.prefix}</span>
                            <span id={`metric-val-${i}`} className="text-7xl md:text-8xl tracking-tighter">0</span>
